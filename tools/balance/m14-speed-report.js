...
function summarize(rows, mode, speed) {
  ...
  const era = Array.from({ length: 7 }, (_, i) => ({
    era: i + 1,
    firstAttemptClear: rate(subset.filter(r => r.firstAttemptWins[i] != null), r => !!r.firstAttemptWins[i]),
    attemptP50: median(subset.map(r => r.attempts[i])),
    attemptP90: q(subset.map(r => r.attempts[i]), 0.9)
  }));
  return {
    mode,
    speed,
...
function acceptance(rows, mode) {
  ...
  const densityRatio = x4.decisionsPerRealMinuteP50 > 0
    ? x8.decisionsPerRealMinuteP50 / x4.decisionsPerRealMinuteP50
    : 1;
  ...
  // M14 is deliberately conservative: x8 is a QoL reward, not a harder input mode.
  const checks = {
  ...
  return {
    ...
    checks,
    metrics: { densityRatio, memoryRatio, earlyClearPenalty, lateAttemptPenalty,
...
function markdown(summaries, decisions) {
  ...
  for (const mode of MODES) {
    ...
    out.push('', '| Era | x1 attempt p50/p90 | x4 attempt p50/p90 | x8 attempt p50/p90 |', '|---:|---:|---:|---:|');
    for (let i = 0; i < 7; i++) {
      const bySpeed = Object.fromEntries(summaries.filter(x => x.mode === mode).map(x => [x.speed, x]));
      const cell = sp => `${num(bySpeed[sp].era[i].attemptP50,0)} / ${num(bySpeed[sp].era[i].attemptP90,0)}`;
