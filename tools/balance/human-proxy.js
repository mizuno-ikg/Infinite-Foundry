...
function isEightUnlockedMeta(meta){return (Number(meta?.upgrades?.automation)||0)>=1}
function effectiveSpeedForState(state,requestedSpeed=1){const requested=normalizeSpeed(requestedSpeed);return requested===8&&!isEightUnlockedMeta(state?.meta)?4:requested}
...
function playCycle(state,{mode='attentive',seed=1,focusPolicy='off',speed=1}={}){
  const r=rng(seed),era=E.currentEra(state),requestedSpeed=normalizeSpeed(speed),simSpeed=effectiveSpeedForState(state,requestedSpeed);let nextDecision=0,decisions=0,buys=0,focusChanges=0;
...
function simulateRoute(seed,{mode='attentive',maxCycles=80,focusPolicy='off',speed=1}={}){
  const requestedSpeed=normalizeSpeed(speed);let state=E.createState(E.baseMeta(seed)),cycles=0,totalGameTime=0,totalRealTime=0,totalDecisions=0,totalBuys=0;const attempts=Array(8).fill(0),firstAttemptWins=Array(8).fill(null),ratios=Array.from({length:8},()=>[]),decisions=Array(8).fill(0),moduleRecoveries=Array.from({length:8},()=>[]),automationUpgrades=Array.from({length:8},()=>[]),memoryByCycle=[],startCreditsByCycle=[],speedByCycle=[];
  while(!state.meta.endingUnlocked&&cycles<maxCycles){
    ...
    const out=playCycle(state,{mode,focusPolicy,speed:requestedSpeed,seed:(seed^Math.imul(cycles+1,0x45d9f3b))>>>0});cycles++;totalGameTime+=E.currentEra(state).duration;totalRealTime+=out.realSeconds;totalDecisions+=out.decisions;totalBuys+=out.buys;ratios[era].push(out.ratio);decisions[era]+=out.decisions;moduleRecoveries[era].push(out.moduleRecoveries);automationUpgrades[era].push(out.automationUpgrades);speedByCycle.push(out.speed);if(attempts[era]===1)firstAttemptWins[era]=!!state.cycle.result.win;
  ...
  return {seed,mode,focusPolicy,requestedSpeed,speed:requestedSpeed,finished:state.meta.endingUnlocked,cycles,totalGameTime,totalRealTime,totalDecisions,totalBuys,decisionsPerRealMinute:realMinutes>0?totalDecisions/realMinutes:0,buysPerRealMinute:realMinutes>0?totalBuys/realMinutes:0,attempts:attempts.slice(1),firstAttemptWins:firstAttemptWins.slice(1),ratios:ratios.slice(1),decisions:decisions.slice(1),moduleRecoveries:moduleRecoveries.slice(1),automationUpgrades:automationUpgrades.slice(1),memoryByCycle,startCreditsByCycle,speedByCycle,foundryMemory:Number(state.meta.foundryMemory)||0,nextBreakthrough:nextBreakthrough(state.meta),upgrades:{...state.meta.upgrades},patentUpgrades:{...state.meta.patentUpgrades}};
}
function simulatePrestigeLoop(seed,{mode='attentive',era=3,maxCycles=12,focusPolicy='off',speed=1}={}){
  const requestedSpeed=normalizeSpeed(speed);let meta=E.baseMeta(seed);meta.era=era;meta.highestEra=Math.max(meta.highestEra,era);let state=E.createState(meta);const rows=[];
...
function summarize(rows){
  ...
  const requestedSpeed=rows[0]?.requestedSpeed||rows[0]?.speed||1;
  return {mode:rows[0]?.mode||'unknown',focusPolicy:rows[0]?.focusPolicy||'off',requestedSpeed,speed:requestedSpeed,finishRate:rows.length?rows.filter(r=>r.finished).length/rows.length:0,cyclesP50:q(rows.map(r=>r.cycles),.5),cyclesP90:q(rows.map(r=>r.cycles),.9),decisionsPerRealMinuteP50:q(rows.map(r=>r.decisionsPerRealMinute),.5),buysPerRealMinuteP50:q(rows.map(r=>r.buysPerRealMinute),.5),finalMemoryP50:q(rows.map(r=>r.foundryMemory||0),.5),firstMeaningfulFailureMemoryP50:q(firstFailureGains,.5),memoryPerCycleP50:q(allMemory.map(x=>x.memoryEarned),.5),era};
}
module.exports={MODES,SPEEDS,normalizeSpeed,isEightUnlockedMeta,effectiveSpeedForState,decisionDelayGameSeconds,playCycle,simulateRoute,simulatePrestigeLoop,summarize,focusWanted};
