...
function playCycle(state,{mode='attentive',seed=1,focusPolicy='off'}={}){
  const r=rng(seed),era=E.currentEra(state);let nextDecision=0,decisions=0,buys=0,focusChanges=0;
...
module.exports={MODES,playCycle,simulateRoute,simulatePrestigeLoop,summarize,focusWanted};
if(require.main===module){
  const seeds=Math.max(6,Number(process.env.IF_HUMAN_SEEDS)||12),maxCycles=Math.max(10,Number(process.env.IF_HUMAN_MAX_CYCLES)||40),focusPolicies=(process.env.IF_HUMAN_FOCUS||'off,losing').split(',').map(x=>x.trim()).filter(Boolean);
  for(const focusPolicy of focusPolicies)for(const mode of ['optimal','attentive','relaxed']){const rows=[];for(let i=1;i<=seeds;i++)rows.push(simulateRoute((0x7100000+i)>>>0,{mode,maxCycles,focusPolicy}));console.log(JSON.stringify(summarize(rows),null,2))}
}
