...
assert(m14.includes("eightButton.dataset.speed='8'"),'x8 speed control must be discoverable in the UI');
assert(m14.includes("(Number(meta?.upgrades?.automation)||0)>=1"),'x8 must unlock only after Automation is acquired');
assert(m14.includes("eightButton.disabled=!unlocked"),'locked x8 must be visibly non-interactive before Automation');
assert(m14.includes("restored.cycle.speed=isEightUnlocked(restored.meta)?8:4"),'a locked save must not restore directly into x8');
assert(m14.includes('Acquire AUTOMATION to unlock ×8 fast-forward'),'locked x8 must explain its unlock condition');
